...
// -----------------------------------------------------------------------------
template<class DType>
__global__ void sign_weighted_minhash(// calc weighted minhash value
    int   batch,                        // number of data points
    int   d,                            // data dimension
    int   sign_size,                    // sign_size = l * k
    const float *d_r_k,                 // hash param r_k ~ Gamma(2,1)
    const float *d_c_k,                 // hash param c_k ~ Gamma(2,1)
    const float *d_b_k,                 // hash param b_k ~ Uniform(0,1)
    const DType *d_set,                 // data set
    const float *d_wgt,                 // data weight
    const u64   *d_pos,                 // data position
    int   *d_sig)                       // hash values of data (return)
{
    // d_sig has <batch> chunks, and each chunk has <sign_size> hash values
    u64 tid = (u64) blockDim.x * blockIdx.x + threadIdx.x;
    if (tid < (u64) batch * sign_size) {
        int did = tid / sign_size;            // get data id
        int num = get_length(did, d_pos);     // get data size
        if (num <= 0) { d_sig[tid] = 0; return; }
        
        const DType *coord = d_set + d_pos[did]; // get coordinates
        const float *weigh = d_wgt + d_pos[did]; // get weights
        
        int hid = tid % sign_size;  // hash func id, hid \in [0,l*k-1]
        int shift = hid * d;        // shift position of hash params
        const float *r_k = d_r_k + shift;
        const float *c_k = d_c_k + shift;
        const float *b_k = d_b_k + shift;
    
        // consistent weighted sampling
        int   id = coord[0];
        float r  = r_k[id], c = c_k[id], b = b_k[id];
        
        int   t  = (int) floor(log(weigh[0])/r + b);
        float y  = exp(r*(t-b));
        float a  = c / (y*exp(r));
        ...
        for (int i = 1; i < num; ++i) {
            id = coord[i]; 
            r  = r_k[id]; c = c_k[id]; b = b_k[id];
            
            t  = (int) floor(log(weigh[i])/r + b);
            y  = exp(r*(t-b));
            a  = c / (y*exp(r));
            
            if (a < min_a) { min_a = a; min_id = id; }
        }
        d_sig[tid] = min_id;
    }
}
    ...
    int   *hash_results)                // hash results (return)
{
    ...
    // init hash parameters 
    int tot_size = d*l*k;
    float *d_r_k;  cudaMalloc((void**)&d_r_k,  sizeof(float)*tot_size);
    float *d_c_k;  cudaMalloc((void**)&d_c_k,  sizeof(float)*tot_size);
    float *d_b_k;  cudaMalloc((void**)&d_b_k,  sizeof(float)*tot_size);
    int   *d_hash; cudaMalloc((void**)&d_hash, sizeof(int)*k);
    
    cudaMemcpy(d_r_k,  r_k, sizeof(float)*tot_size, cudaMemcpyHostToDevice);
    cudaMemcpy(d_c_k,  c_k, sizeof(float)*tot_size, cudaMemcpyHostToDevice);
    cudaMemcpy(d_b_k,  b_k, sizeof(float)*tot_size, cudaMemcpyHostToDevice);
    cudaMemcpy(d_hash, uni_hash, sizeof(int)*k,     cudaMemcpyHostToDevice);
    
    // mem_avail = totoal_size - memory(d_coeA + d_coeB + d_hash)
    u64 mem_avail = GPU_MEMORY_LIMIT - (sizeof(float)*tot_size*3+sizeof(int)*k);
    u64 mem_usage = 0UL, n_set = 0UL;
    int n_pos = 0, batch = 0, start = 0;
    ...
    for (int i = 0; i <= n; ++i) {
        ...
        // ---------------------------------------------------------------------
        if (i < n) {
            // d_set + d_wgt + d_pos + d_sig + d_val + d_res
            n_set = datapos[i+1] - datapos[start];
            n_pos = batch + 2;
            mem_usage = (sizeof(DType) + sizeof(float))*n_set + 
                sizeof(u64)*n_pos + sizeof(int)*(l*k+l+m)*(batch+1);
        }
        // ---------------------------------------------------------------------
        //  parallel minhash for bucket if over mem_avail or end
        // ---------------------------------------------------------------------
        if (mem_usage > mem_avail || i == n) {
            ...
            n_pos = batch + 1;
            mem_usage = (sizeof(DType) + sizeof(float))*n_set + 
                sizeof(u64)*n_pos + sizeof(int)*(l*k+l+m)*batch;
#ifdef DEBUG_INFO
            printf("Rank #%d: n=%d, i=%d, batch=%d, mem_usage=%lu, mem_avail=%lu\n", 
                rank, n, i, batch, mem_usage, mem_avail);
#endif
            ...
            const DType *h_set = dataset + datapos[start]; // data set at host
            const float *h_wgt = datawgt + datapos[start]; // data weight at host
            u64   *h_pos = new u64[n_pos];                 // data postion at host
            copy_pos(n_pos, datapos+start, h_pos);

            // cuda allocation: data set, data weight, data position
            DType *d_set; cudaMalloc((void**) &d_set, sizeof(DType)*n_set);
            float *d_wgt; cudaMalloc((void**) &d_wgt, sizeof(float)*n_set);
            u64   *d_pos; cudaMalloc((void**) &d_pos, sizeof(u64)*n_pos);
            ...
            cudaMemcpy(d_set, h_set, sizeof(DType)*n_set, cudaMemcpyHostToDevice);
            cudaMemcpy(d_wgt, h_wgt, sizeof(float)*n_set, cudaMemcpyHostToDevice);
            cudaMemcpy(d_pos, h_pos, sizeof(u64)*n_pos,   cudaMemcpyHostToDevice);
            
            // calc minhash values for batch data by GPUs
            int block = BLOCK_SIZE;
            int grid  = ((u64) batch*l*k + block-1) / block;
            sign_weighted_minhash<DType><<<grid, block>>>(batch, d, l*k, d_r_k, 
                d_c_k, d_b_k, d_set, d_wgt, d_pos, d_sig);
            
            // calc the k-concatenation by GPUs
            grid = ((u64) batch*l + block-1) / block;
            sign_concat<<<grid, block>>>(batch, l, k, prime, d_sig, d_hash, d_val);
            ...
            // release space
            cudaFree(d_set); cudaFree(d_wgt); cudaFree(d_pos);
            cudaFree(d_sig); cudaFree(d_val); cudaFree(d_res);
            delete[] h_pos;  delete[] h_res;
    ...
    // release space
    assert(start == n);
    cudaFree(d_r_k); cudaFree(d_c_k); cudaFree(d_b_k); cudaFree(d_hash);
}

// -----------------------------------------------------------------------------
template void weighted_minhash(     // weighted minwise hashing (pair-wise)
    int   rank,                         // MPI rank
    int   n,                            // number of data points
    int   d,                            // data dimension
    int   prime,                        // prime number for data
