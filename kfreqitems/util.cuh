...
// -----------------------------------------------------------------------------
template<class DType>
__host__ __device__ float weighted_jaccard_dist(// calc weighted jaccard dist
    int   n_data,                       // number of data dimensions
    int   n_seed,                       // number of seed dimensions
    float data_sum,                     // data sum of weights
    float seed_sum,                     // seed sum of weights
    const DType *data_coord,            // data coordinate
    const float *data_weigh,            // data weight
    const int   *seed_coord,            // seed coordinate
    const float *seed_weigh)            // seed weight
{
    // float data_sum = 0.0f;
    // for (int i = 0; i < n_data; ++i) data_sum += data_weigh[i];
    
    // float seed_sum = 0.0f;
    // for (int j = 0; j < n_seed; ++j) seed_sum += seed_weigh[j];
    
    int i = 0, j = 0; // i for data, j for seed
    float overlap = 0.0f;
    while (i < n_data && j < n_seed) {
        if (data_coord[i] < seed_coord[j]) {
            ++i;
        } else if (seed_coord[j] < data_coord[i]) {
            ++j;
        } else { 
            if (data_weigh[i] <= seed_weigh[j]) {
                overlap += data_weigh[i];
            } else {
                overlap += seed_weigh[j];
            }
            ++i; ++j;
        }
    }
    return 1.0f - overlap / (data_sum + seed_sum - overlap);
}
...
// -----------------------------------------------------------------------------
template<class DType>
void distinct_coord_weigh_freq(     // get distinct coordinates, weights & freqs
    u64   total_num,                    // total number of coordinates
    DType *arr_coord,                   // array of all coordinates (allow modify)
    float *arr_weigh,                   // array of all weights (allow modify)
    DType *coord,                       // distinct coordinates (return)
    float *weigh,                       // sum of weights (return)
    int   *freq,                        // sum of frequencies (return)
    int   &cnt,                         // number of distinct coords (return)
    int   &max_freq,                    // maximum frequency (return)
    float &max_weight)                  // maximum weight (return)
{
    // sort all coordinates and weights in ascending order
    thrust::sort_by_key(arr_coord, arr_coord + total_num, arr_weigh);
    
    // get the distinct coordinates and sum of weights & frequencies (sequential)
    cnt = 0; max_freq = 0; max_weight = 0.0f;
    
    int last = 0; weigh[cnt] = 0.0f;
    for (size_t i = 1; i < total_num; ++i) {
        weigh[cnt] += arr_weigh[i-1];
        
        if (arr_coord[i] != arr_coord[i-1]) {
            freq[cnt]  = i - last;
            coord[cnt] = arr_coord[i-1];
            if (freq[cnt]  > max_freq)   max_freq   = freq[cnt];
            if (weigh[cnt] > max_weight) max_weight = weigh[cnt];
            
            last = i; ++cnt; weigh[cnt] = 0.0f;
        }
    }
    // deal with the last element
    weigh[cnt] += arr_weigh[total_num-1];
    freq[cnt]   = total_num - last;
    coord[cnt]  = arr_coord[total_num-1];
    if (freq[cnt]  > max_freq)   max_freq   = freq[cnt];
    if (weigh[cnt] > max_weight) max_weight = weigh[cnt];
    ++cnt;
}
