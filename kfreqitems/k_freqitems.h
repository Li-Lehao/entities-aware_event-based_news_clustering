...
protected:
    ...
    float *datasum_;                // sum of data weights
...
KFreqItems<DType>::KFreqItems(      // constructor
    ...
    galpha_(galpha), lalpha_(lalpha), dataset_(dataset), datawgt_(datawgt), 
    datapos_(datapos)
{
    ...
    labels_  = new int[n]; // init labels_
...
void output_centers(                // output k centers as seeds
    ...
    const std::vector<f32> &seedwgt,    // seed weight (return)
    ...
    const char *folder)                 // output folder
{
    ...
    fwrite(seedwgt.data(), sizeof(float), seedpos[k], fp);
    fclose(fp);
}
    ...
    const char *folder)                 // output folder
{
    ...
    // output labels
    char fname[200]; sprintf(fname, "%s_%d.txt", prefix, rank);
    FILE *fp = fopen(fname, "wb");
    if (!fp) { printf("Could not open %s\n", fname); exit(1); }
    ...
    float lbeta)                        // local  beta
{
    ...
    if (seeding_algo == 0) {
        ...
        random_seeding<DType>(rank_, size_, n_, N_, avg_d_, k, dataset_, 
            datawgt_, datapos_, datasum_, distinct_ids, seedset_, seedwgt_, 
            seedpos_, seedsum_);
        // output_distinct_ids(rank_, k, distinct_ids, "random", folder_);
    }
    else if (seeding_algo == 1) {
        // k-means++ seeding (use OpemMP by default, hard_device=0)
        int *weights = new int[N_]; memset(weights, 1, sizeof(int)*N_);
        kmeanspp_seeding<DType>(rank_, size_, n_, N_, avg_d_, k, 0, dataset_, 
            datawgt_, datapos_, datasum_, weights, distinct_ids, seedset_, 
            seedwgt_, seedpos_, seedsum_);
        // output_distinct_ids(rank_, k, distinct_ids, "kmeanspp", folder_);
        delete[] weights;
    }
    else if (seeding_algo == 2) {
        // k-means|| seeding (use GPUs by default, hard_device=1)
        int l = k;
        int t = 5;
        kmeansll_seeding<DType>(rank_, size_, n_, N_, avg_d_, k, l, t, 1, 
            dataset_, datawgt_, datapos_, datasum_, distinct_ids, seedset_, 
            seedwgt_, seedpos_, seedsum_);
        ...
        k = silk_seeding<DType>(rank_, size_, n_, N_, d_, avg_d_, k, m1, h1, 
            t, m2, h2, b, delta, gbeta, lbeta, galpha_, lalpha_, prefix_, 
            dataset_, datawgt_, datapos_, datasum_, seedset_, seedwgt_, 
            seedpos_, seedsum_);
        // output_centers(rank_, k, seedset_, seedwgt_, seedpos_, "silk", folder_);
    }
    else {
        exit(1);
    }
    ...
    for (int iter = 1; iter <= max_iter_; ++iter) {
        ...
        exact_assign_data<DType>(rank_, n_, K, dataset_, datawgt_, datapos_, 
            datasum_, seedset_.data(), seedwgt_.data(), seedpos_.data(), 
        ...
        printf("Rank #%d: K=%d (before), ", rank_, K);
    ...
    char fname[200];
    sprintf(fname, "%slabels_%d.txt", folder_, k);
    FILE *fp = fopen(fname, "w");
    for (int i = 0; i < n_; ++i) {
        fprintf(fp, "%d\n", labels_[i]);
    }
    ...
    float &mse)                         // mse (return)
{
    ...
    calc_local_stat_by_seeds<DType>(n_, K, labels_, dataset_, datawgt_, 
