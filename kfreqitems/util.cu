...
// -----------------------------------------------------------------------------
float calc_sum_weights(             // calc weighted jaccard dist of data & seed
    int   did,                          // data id
    ...
    const u64   *datapos)               // data position
{
    ...
    float sum = 0.0f;
    for (int i = 0; i < n_data; ++i) sum += data_weigh[i];
...
int gather_all_buffers(             // gather buffers from diff threads to root
    int   size,                         // number of MPIs (size)
    const std::vector<float> &buffer,   // buffer in local
    float *all_buff)                    // all buffers @root (return)
{
    // gather the length of different buffers from different threads
    int len = (int) buffer.size(); // get the length of this buffer
    int *rlen = new int[size];
    ...
    // get the total length of buffer weights (wlen) from all threads
    int wlen = 0;
    for (int i = 0; i < size; ++i) wlen += rlen[i];
    assert(wlen <= MAX_BUFF_SIZE);
    ...
    MPI_Barrier(MPI_COMM_WORLD);
    MPI_Gatherv(buffer.data(), len, MPI_FLOAT, all_buff, rlen, displs, 
        MPI_FLOAT, 0, MPI_COMM_WORLD); // 0 is the root location
    MPI_Barrier(MPI_COMM_WORLD);
...
// -----------------------------------------------------------------------------
void all_buff_to_cand_list(         // convert all buffers into cand list
    int   n_buff,                       // length of all buffers
    int   n_weight,                     // length of all buffers weights
    const int   *all_buff,              // all buffers
    const float *all_buff_weights,      // all buffers weights
    std::unordered_map<int, std::vector<Triplet> > &cand_list)
{
    int i = 0;
    int j = 0;
    while (i < n_buff && j < n_weight) {
        int   key = all_buff[i++];
        int   len = all_buff[i++];
        
        const int *coord = &all_buff[i]; i += len;
        const int *freq  = &all_buff[i]; i += len;
        const f32 *weigh = &all_buff_weights[j]; j += len;
        
        // Note: the value of cand_list[key] may be duplicated
        std::vector<Triplet> &tmp = cand_list[key];
        tmp.reserve(tmp.size() + len);
        for (int k = 0; k < len; ++k) {
            Triplet t;
            t.coord_ = coord[k]; t.freq_ = freq[k]; t.weigh_ = weigh[k];
            tmp.push_back(t);
        }
    }
    assert(i == n_buff);
...
// -----------------------------------------------------------------------------
void broadcast_seeds(               // broadcast seeds to other threads
    int   rank,                         // MPI rank
    int   size,                         // number of MPIs (size)
    std::vector<int> &seedset,          // seed set (return)
    std::vector<f32> &seedwgt,          // seed weights (return)
    std::vector<u64> &seedpos,          // seed position (return)
    std::vector<f32> &seedsum)          // seed sum of weights (return)
{
    // broadcast seedpos from root to other threads
    u64 seedpos_size = seedpos.size();
    MPI_Barrier(MPI_COMM_WORLD);
    MPI_Bcast(&seedpos_size, 1, MPI_UINT64_T, 0, MPI_COMM_WORLD);
    MPI_Barrier(MPI_COMM_WORLD);
    
    if (rank > 0) seedpos.resize(seedpos_size);
    
    assert(seedpos_size <= MAX_INT);
    MPI_Barrier(MPI_COMM_WORLD);
    MPI_Bcast(seedpos.data(), seedpos_size, MPI_UINT64_T, 0, MPI_COMM_WORLD);
    MPI_Barrier(MPI_COMM_WORLD);
    
    // broadcast seedsum from root to other threads
    u64 seedsum_size = seedsum.size();
    MPI_Barrier(MPI_COMM_WORLD);
    MPI_Bcast(&seedsum_size, 1, MPI_UINT64_T, 0, MPI_COMM_WORLD);
    MPI_Barrier(MPI_COMM_WORLD);
    
    if (rank > 0) seedsum.resize(seedsum_size);
    
    assert(seedsum_size <= MAX_INT);
    MPI_Barrier(MPI_COMM_WORLD);
    MPI_Bcast(seedsum.data(), seedsum_size, MPI_FLOAT, 0, MPI_COMM_WORLD);
    MPI_Barrier(MPI_COMM_WORLD);
    
    // broadcast seedset from root to other threads
    u64 seedset_size = seedset.size();
    MPI_Barrier(MPI_COMM_WORLD);
    MPI_Bcast(&seedset_size, 1, MPI_UINT64_T, 0, MPI_COMM_WORLD);
    MPI_Barrier(MPI_COMM_WORLD);
    
    if (rank > 0) seedset.resize(seedset_size);
    
    int batch = MAX_BUFF_SIZE;
    for (size_t i = 0; i < seedset_size; i += batch) {
        if (i+batch > seedset_size) batch = seedset_size-i;

        MPI_Barrier(MPI_COMM_WORLD);
        MPI_Bcast(seedset.data()+i, batch, MPI_INT, 0, MPI_COMM_WORLD);
        MPI_Barrier(MPI_COMM_WORLD);
    }
    
    // broadcast seedwgt from root to other threads
    u64 seedwgt_size = seedwgt.size();
    MPI_Barrier(MPI_COMM_WORLD);
    MPI_Bcast(&seedwgt_size, 1, MPI_UINT64_T, 0, MPI_COMM_WORLD);
    MPI_Barrier(MPI_COMM_WORLD);
    
    if (rank > 0) seedwgt.resize(seedwgt_size);
    
    batch = MAX_BUFF_SIZE;
    for (size_t i = 0; i < seedwgt_size; i += batch) {
        if (i+batch > seedwgt_size) batch = seedwgt_size-i;

        MPI_Barrier(MPI_COMM_WORLD);
        MPI_Bcast(seedwgt.data()+i, batch, MPI_FLOAT, 0, MPI_COMM_WORLD);
        MPI_Barrier(MPI_COMM_WORLD);
    }
}
